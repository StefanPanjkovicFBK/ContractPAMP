...
class Solver():
            ...
            if len(seq) <= len(state.path):
                for i in range(0, len(state.path)-len(seq)+1):
                    ...
                    for j in range(i, i+len(seq)):
                        seq_event, seq_id, lifted = seq[j-i]
                        path_event, path_id = state.path[j]
                        if lifted:
                            liftedActionSeq = self.get_lifted_name(seq_event.action)
                            liftedActionPath = self.get_lifted_name(path_event.action)
                            if (liftedActionSeq != liftedActionPath) or (seq_event.pos != path_event.pos):
                                match = False
                                break
                        else:
    ...
    def validate(self, tn, path, planningProblem, platform):
        ...
        for i in range(0, len(overlappingSeqs)):
            ...
            if tuple(contractList) in self.contract_cache:
                composedContract = (self.contract_cache)[tuple(contractList)]
                currentTimepoint += len(seq)
                if i < len(overlappingSeqs)-1:
                    continue
            else:
        ...
        while True:
            ...
            if tn is not None:
                ...
                if outcome:
                    ...
                    if seq != None:
                        self.bad_sequences.append(self.generalize_seq(seq))
            else:
                print("problem unsolvable")
